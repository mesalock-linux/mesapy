...
{
    // fix reason: double to long could be overflow
    if(MAX_STACK_SIZE * fraction > LONG_MIN && MAX_STACK_SIZE * fraction < LONG_MAX)
...
int main(void){
    double fraction = tis_double_interval(DBL_MIN, DBL_MAX);
    long k = tis_long_interval(LONG_MIN, LONG_MAX);    
    LL_stack_set_length_fraction(fraction);
    LL_stack_too_big_slowpath(k);
}
