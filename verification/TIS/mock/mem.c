...
void pypy_debug_alloc_stop(void *addr)
{
  ...
  if (!addr)
	return;
  struct pypy_debug_alloc_s *p;
  //pypy_debug_alloc_list = malloc(sizeof(struct pypy_debug_alloc_s));
  //if(pypy_debug_alloc_list==NULL) return;
  //pypy_debug_alloc_list->addr = addr;
...
/*
int main(void){
    //tis_make_unknown(pypy_debug_alloc_list, sizeof pypy_debug_alloc_list);
    pypy_check_stack_count();
    struct boehm_fq_s *fg = malloc(sizeof(void *) * 2);
    //boehm_fq_next_dead(&fg);
    free(fg);
    char addr[100];
    tis_make_unknown(addr, sizeof addr);
    char funcname[100];
    tis_make_unknown(funcname, sizeof funcname);
    pypy_debug_alloc_start(addr, funcname);
    pypy_debug_alloc_stop(addr);
    pypy_debug_alloc_results();
    //free(pypy_debug_alloc_list);
}
*/
