...
static void skiplist_insert(skipnode_t *head, skipnode_t *new)
{
    ...
    unsigned long height_of_new = (unsigned long)((new->data - ((char *)new + size0)) /
        sizeof(skipnode_t *));
    
    unsigned long level = SKIPLIST_HEIGHT - 1;
    //fix_reason: new could be nullptr
    if(new == NULL || head == NULL) return;
...
{
    unsigned long level = SKIPLIST_HEIGHT - 1;
    //fix_reason: head could be nullptr
    if(head == NULL) return head;
    while (1) {
        skipnode_t *next = head->next[level];
