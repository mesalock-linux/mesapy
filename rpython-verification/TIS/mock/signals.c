...
int pypysig_poll(void)
{
    ...
    for (index = 0; index < N_LONGSIG; index++) {
      ...
      retry:
        ...
        if (value != 0L) {
            int j = 0;
            // fix reason: invalid shift 
            while (j<32 && (value & (1UL << j)) == 0)
                j++;
            if(j==32) return -1;
...
int main(void) {
    int signum = tis_int_interval(-2147483648, 2147483647);
    pypysig_default(signum);
    pypysig_pushback(signum);
    signal_setflag_handler(signum);
    pypysig_setflag(signum);
    pypysig_getaddr_occurred();
    pypysig_reinstall(signum);
    pypysig_poll();
    pypysig_set_wakeup_fd(signum, signum);
}
