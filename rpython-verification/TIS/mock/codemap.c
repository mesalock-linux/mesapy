...
long *pypy_jit_codemap_del(unsigned long addr, unsigned int size)
{
    ...
    //fix reason:  node could be nullptr
...
void *pypy_find_codemap_at_addr(long addr, long* start_addr)
{
    ...
    // fix reason: codemap could be NULL
                                ...
                                long *current_pos_addr)
{
    ...
    // fix reason: codemap and data could be nullptr
    if(codemap == NULL) return -1;
...
int main(void){
    unsigned long addr = tis_unsigned_long_interval(0,4294967295);
    unsigned int machine_code_size = tis_unsigned_int_interval(0,65535);
    long bi = tis_long_interval(-2147483648, 2147483647);
    long *bytecode_info = &bi;
    unsigned int bytecode_info_size = tis_unsigned_int_interval(0,65535);
    //skipnode_t *m;
    //tis_make_unknown(m, sizeof m);
    pypy_jit_codemap_add(addr, machine_code_size, bytecode_info, bytecode_info_size);
    pypy_jit_codemap_del(addr, machine_code_size);
    pypy_jit_codemap_firstkey();
    //pypy_yield_codemap_at_addr();
    pypy_find_codemap_at_addr(addr, &bi);
    //pypy_codemap_invalid_set(m, addr, &bi);
}
