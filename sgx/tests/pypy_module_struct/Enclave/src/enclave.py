...
def test_struct_impl():
    t = AppTestStruct()
    for k, v in AppTestStruct.__dict__.items():
        if k.startswith("test_"):
            print "[+] testing", k
            v(t)

def test_struct_buffer():
    t = AppTestStructBuffer()
    for k, v in AppTestStructBuffer.__dict__.items():
        if k.startswith("test_"):
            print "[+] testing", k
            v(t)
...
def test_fast_path():
    t = AppTestFastPath()
    for k, v in AppTestFastPath.__dict__.items():
        if k.startswith("test_"):
            print "[+] testing", k
            v(t)
...
class AppTestStruct():
    def test_error(self):
        ...
        assert struct.error.__mro__ == (struct.error, Exception,
    ...
    def test_bool(self):
        ...
        assert pack("@?", True) == '\x01'
        assert pack("@?", False) == '\x00'
        assert struct.unpack("?", 'X')[0] is True
    ...
    def test_unicode(self):
        ...
        if '__pypy__' not in sys.builtin_module_names:
            skip("PyPy extension")
        data = struct.pack("uuu", u'X', u'Y', u'Z')
        assert data == str(buffer(u'XYZ'))
        assert struct.unpack("uuu", data) == (u'X', u'Y', u'Z')

    def test_unpack_buffer(self):
        """
        Buffer objects can be passed to struct.unpack().
        """
        b = buffer(struct.pack("ii", 62, 12))
    ...
    def test_pack_unpack_buffer(self):
        ...
        for offset in [2, -17]:
            ...
            assert str(buffer(b)) == ('\x00' * 2 +
                                      ...
                                      '\x00' * (19-sz-2))
        exc = raises(TypeError, struct.pack_into, "ii", buffer(b), 0, 17, 42)
        exc = raises(TypeError, struct.pack_into, "ii", 'test', 0, 17, 42)
        ...
        assert struct.unpack_from("ii", buffer(b, 2)) == (17, 42)
        assert struct.unpack_from("ii", buffer(b), 2) == (17, 42)
        assert struct.unpack_from("ii", memoryview(buffer(b)), 2) == (17, 42)
        exc = raises(TypeError, struct.unpack_from, "ii", 123)
        exc = raises(struct.error, struct.unpack_from, "ii", None)
        exc = raises(struct.error, struct.unpack_from, "ii", '')
        exc = raises(struct.error, struct.unpack_from, "ii", memoryview(''))
...
class AppTestFastPath(object):
    def test_unpack_simple(self):
        buf = struct.pack("iii", 0, 42, 43)
        assert struct.unpack("iii", buf) == (0, 42, 43)

    def test_unpack_from(self):
        buf = struct.pack("iii", 0, 42, 43)
        offset = struct.calcsize("i")
        assert struct.unpack_from("ii", buf, offset) == (42, 43)

    def test_unpack_bytearray(self):
        data = struct.pack("iii", 0, 42, 43)
        buf = bytearray(data)
        assert struct.unpack("iii", buf) == (0, 42, 43)
    ...
    def test_pack_into_bytearray(self):
        expected = struct.pack("ii", 42, 43)
        buf = bytearray(len(expected))
        struct.pack_into("ii", buf, 0, 42, 43)
        assert buf == expected

    def test_pack_into_bytearray_padding(self):
        expected = struct.pack("xxi", 42)
        buf = bytearray(len(expected))
        struct.pack_into("xxi", buf, 0, 42)
        assert buf == expected
